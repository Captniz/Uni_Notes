...
Per convertire da decimale a binario separo la parte **intera** da quella **frazionaria** e converto quella intera normalmente.
...
#### Spazio dei valori di rappresentazione
Quindi *$x$* può essere rappresentato in **virgola  mobile** su *$k$ bit totali* con : 
- *$m$* bit di **mantissa**
- *$e = k-m$* bit di **esponente**
#### Calcolo dei valori di rappresentazione
...
##### Mantissa
Per calcolare la mantissa si inizia con il calcolare la codifica in virgola fissa del numero.

A questo punto si sposta la virgola dopo la prima cifra ( *Che sarà sempre 1, guarda callout* ), **normalizzando** il numero. 
La mantissa sarà quindi la **parte dopo la virgola del numero.**

Con questo calcolo troviamo anche $E'$ , cioè quante volte muoviamo la virgola a **destra o a sinistra**; quando la muoviamo a *sinistra* $E'$ è **positivo**, al contrario muovendola a *destra* $E'$ è **negativo**.
...
> Nel caso in cui i numeri che vogliamo rappresentare siano molto piccoli dobbiamo utilizzare i numeri **denormalizzati**, di cui parleremo dopo.
...
###### Converto in binario le due parti
> $(0)_{10}$ -> $(0)_2$
> $(12345)_{10}$ -> $(00011111100110100111)_2$
###### Unisco le parti e normalizzo il numero
> $(0.00011111100110100111)$ -> $(1.1111100110100111)\cdot2^{-4}$
###### Converto l'esponente in binario
> $E=-4+(2^{e-1}-1)$ -> $E=-4+127_{float\ bias}=123$ 
...
A differenza di un numero normalizzato, in cui $E=(E'+bias)$ nei numeri denormalizzati **$E=(1-bias)$**.

Inoltre i numeri denormalizzati **NON HANNO** il primo **bit implicito a $1$**, bensì **lo hanno a $0$**.
